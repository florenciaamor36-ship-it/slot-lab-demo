...
function App() {
  const [selected, setSelected] = useState(0); const [bet, setBet] = useState(100); const [balance, setBalance] = useState(10000);
  const [result, setResult] = useState<SlotSpinResult | null>(null); const [spinning, setSpinning] = useState(false); const [showBigWin, setShowBigWin] = useState(false);
  const game = games[selected]; const isCleopatra = selected === 0;
  const engine = useMemo(() => new SlotEngine(game), [game]);
  const spin = () => {
    if (spinning || balance < bet) return;
    audio.play('spin'); setSpinning(true); setResult(null); setShowBigWin(false);
    window.setTimeout(() => { const r = engine.spin(bet, 'demo-only'); setBalance((b) => b - bet + r.totalPayout); setResult(r); setSpinning(false); audio.play(r.isJackpot ? 'jackpot' : r.isWin ? 'win' : 'stop'); if (r.isJackpot || r.totalMultiplier >= 20) setShowBigWin(true); }, 900);
  };
  const grid = result?.grid ?? [];
  return <div className={`app ${isCleopatra ? 'cleopatra-app' : ''}`}>
    <header><b>🎰 SLOT LAB DEMO</b><span>Motor común SlotEngine · fichas virtuales · no dinero real</span></header>
    <main><aside><h2>Máquinas</h2>{games.map((g, i) => <button className={i === selected ? 'active' : ''} onClick={() => { setSelected(i); setResult(null); setShowBigWin(false); }} key={g.id}>{g.title}<small>{g.subtitle}</small></button>)}</aside>
      <section>
        ...
        {!isCleopatra && <div className={`grid ${spinning ? 'spinning' : ''} ${result?.isWin ? 'winner' : ''}`}>{(grid.length ? grid.flat() : Array.from({ length: 15 }, () => null)).map((s, i) => <div className="cell" key={i} style={{ color: s?.color }}>{s?.emoji ?? '❔'}<small>{s?.label}</small></div>)}</div>}
        <div className="controls"><span>Saldo demo: <b>{balance.toLocaleString('es-AR')}</b></span><label>Apuesta <input type="number" value={bet} min="10" max="1000" step="10" onChange={(e) => setBet(Math.max(10, Number(e.target.value) || 10))} /></label>{!isCleopatra && <button className="spin" onClick={spin} disabled={spinning}>{spinning ? 'GIRANDO…' : 'GIRAR'}</button>}</div>
        <div className={`message ${result?.isWin ? 'win' : ''}`}>{result?.summary ?? 'Elegí una máquina y girá para probarla.'}</div>
        <p className="note">Demo matemática con fichas virtuales. El resultado y la validación no representan dinero real ni un sistema de premios conectado.</p>
      </section>
    </main>
    {showBigWin && result && <CleopatraOverlay result={result} onClose={() => setShowBigWin(false)} />}
  </div>;
}
